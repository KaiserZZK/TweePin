...
var recenterButton = document.getElementById('centerButton');
...
recenterButton.addEventListener('click', ()=> {
    // console.log('hehe')
    // map.panTo({ lat: 40.80636260000001, lng: -73.9631912});  
    // myLatLng = { lat: 40.80636260000001, lng: -73.9631912};
    ...
    var query = document.getElementById('myCenter').value;
...
function initMap() {
    myLatLng = { lat: 40.8044028, lng: -73.9661071 };//{ lat: 40.8044028, lng: -73.9661071};
    ...
    // google.maps.event.addListener(button_s, 'click', function(event) {
        ...
        for (let i = 0; i < markers.length; i++){
            ...
            newMarker = createMarker(currentMarker[1], currentMarker[0], currentMarker[2], currentMarker[3], currentMarker[4]);
...
function handleResults() {
    ...
    for (var obj of responseJSON) {
        ...
        markers.push([obj.tweet, obj.streets, +obj.latitude, +obj.longitude, obj.link]);
        // console.log(markers);
    }
...
function createMarker(title, content, latitude, longitude, link) {
    let text = "</br></br>Original Tweet";
    let linkToTweet = text.link(link);
    ...
    const infowindow = new google.maps.InfoWindow({
        content: content + linkToTweet + viewDiscussion,
    });
